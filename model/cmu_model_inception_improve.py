...
def STEM_block(input_tensor, filters, stage, weight_decay):
    ...
    x4 = relu(x4)
    shortcut = conv(input_tensor, filters4[1], 1, conv_name_base + '1', weight_decay, strides=(2, 2))
...
def tiny_inception_block(input_tensor, filters, stage,branch, weight_decay):
    ...
    x2 = conv(x, filters2[0], 3, conv_name_base + 'b1', weight_decay)
    ...
    x3 = conv(x, filters3[0], 3, conv_name_base + 'c1', weight_decay)
...
def stage1_block(x, num_p, branch, weight_decay):
    ...
    x = BatchNormalization(axis=bn_axis, epsilon=1e-5, momentum=0.9)(x)
    x = relu(x)
    x= tiny_inception_block(x, [[64],[64,64],[64, 64, 128]], 3,branch, (weight_decay, 0))
...
def stageT_block(x, num_p, stage, branch, weight_decay):
    ...
    x = add([x1, x])
    x = relu(x)
    x = conv(x, 256, 3, "Mconv4_stage%d_L%d" % (stage, branch), (weight_decay, 0))
    x = BatchNormalization(axis=bn_axis, epsilon=1e-5, momentum=0.9)(x)
    x = relu(x)
    x = tiny_inception_block(x, [[64], [64, 64], [64, 64, 128]], stage+2,branch, (weight_decay, 0))
