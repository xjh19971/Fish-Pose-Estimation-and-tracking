...
def tiny_inception_block(input_tensor, filters, stage, weight_decay):
    ...
    x2 = conv(input_tensor, filters3[0], 3, conv_name_base + 'd1', weight_decay)
    x2 = BatchNormalization(axis=bn_axis, name=bn_name_base + 'd1', epsilon=1e-5, momentum=0.9)(x2)
    x2 = relu(x2)
    x2 = conv(x2, filters3[1], 3, conv_name_base + 'd2', weight_decay)
    x2 = BatchNormalization(axis=bn_axis, name=bn_name_base + 'd2', epsilon=1e-5, momentum=0.9)(x2)
    x2 = relu(x2)
...
def stageT_block(x, num_p, stage, branch, weight_decay):
    ...
    x = BatchNormalization(axis=bn_axis, epsilon=1e-5, momentum=0.9)(x)
    x = relu(x)
    x = tiny_inception_block(x, [[64], [64, 64], [64, 64, 64]], stage+2, (weight_decay, 0))
