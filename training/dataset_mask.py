...
AUGMENTORS_LIST = [
        ...
        CropAug(320, 320, 180, 180, center_perterb_max=0, border_value=(128, 128, 128), #40
                 mask_border_val=1),
...
def build_sample(components):
    ...
    heatmap = create_heatmap(JointsLoader.num_joints_and_bkg, int(meta.crop_y_max / stride),
                             ...
                             int(meta.crop_y / stride), int(meta.crop_x / stride), meta.aug_joints, 4.00, stride=stride)

    pafmap = create_paf(JointsLoader.num_connections, int(meta.crop_y_max / stride), int(meta.crop_x_max / stride),
                        int(meta.crop_y / stride), int(meta.crop_x / stride), meta.aug_joints, 1, stride=stride)
    ...
    return [image.astype(np.uint8), mask_paf, mask_heatmap, pafmap, heatmap, maskmap, numlist]
