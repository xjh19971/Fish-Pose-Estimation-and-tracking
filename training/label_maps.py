...
def create_mask(num_maps_heatmap, num_maps_paf, height, width, cropheight, cropwidth, all_joints, sigma,threshold, stride):
    n=0
    for joints in all_joints:
        for plane_idx, joint in enumerate(joints):
            if joint:
                n=n+1
                break
    maskmap = np.zeros((height, width, n), dtype=np.float64)
    numlist = np.zeros(n, dtype=np.float64)
    n = 0
    for joints in all_joints:
        maskmaptemp = np.zeros((height, width), dtype=np.float64)
        for plane_idx, joint in enumerate(joints):
            if joint:
                _put_heatmap_mask_on_plane(maskmaptemp, joint, sigma, cropheight, cropwidth, stride,
                                      height / 2, width / 2)
        ...
        for plane_idx, (j_idx1, j_idx2) in enumerate(JointsLoader.joint_pairs):
            ...
            _put_paf_mask_on_plane(maskmaptemp, x1, y1, x2, y2,
                              threshold, cropheight, cropwidth,height/2,width/2)
        num=np.sum(maskmaptemp==1)
        maskmap[:,:,n]=maskmaptemp
        numlist[n]=num
        n=n+1
    return maskmap,numlist
...
def _put_heatmap_mask_on_plane(heatmap, joint, sigma, height, width, stride, mapcentery, mapcenterx):
    start = stride / 2.0 - 0.5
...
def _put_paf_mask_on_plane(vectormap, x1, y1, x2, y2,
                     threshold, height, width, mapcentery, mapcenterx):
    min_x = max(int(mapcenterx - width / 2), int(round(min(x1, x2) - threshold)))
    max_x = min(int(mapcenterx + width / 2), int(round(max(x1, x2) + threshold)))
